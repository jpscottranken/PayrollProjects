...
{
    public partial class frmPayroll07 : Form
    {
        public frmPayroll07()
        {
            InitializeComponent();
        ...
        private void CalculateCurrentEmployeeGrossPay()
        {
            decimal hours = 0m;
            decimal rate = 0m;
            string msg = "";
        ...
        private bool IsValidData()
        {
            ...
            // Validate the department text box
        ...
        private void FillTextBoxes(int emp)
        {
            ...
            cboDepartment.SelectedItem  = empDepartments[emp];

            CalculateYearsOfService(empHireDates[emp]);

            if (empHoursWorked[emp] >= 40)
            {
        ...
        private void CalculateYearsOfService(DateTime hDate)
        {
            ...
            DisplayStatsInListBox();
        }

        private void DisplayStatsInListBox()
        {
            CalculateCurrentEmployeeGrossPay();
            lstStats.Items.Insert(0, "\t\tEmployee " +
                                txtEmployeeID.Text + " " +
                                "Statistics");
            lstStats.Items.Insert(1, "\t\t-------------------------------------");
            lstStats.Items.Insert(2, "Name: "                + txtFirstName.Text +
                                    " "                      + txtLastName.Text  +
                                    "\t\tDeptartment: "      + cboDepartment.SelectedItem);
            lstStats.Items.Insert(3, "Hire Date: "           + txtHireDate.Text  +
                                     "\tYears Of Service: "   + txtYearsOfService.Text);
            lstStats.Items.Insert(4, "Hrs Worked: "          + txtHoursWorked.Text.ToString()
                                + "\tHourly Rate: "          + txtHourlyRate.Text.ToString()
        ...
        private void frmPayroll07_Load(object sender, EventArgs e)
        {
            cboDepartment.SelectedIndex = 0;
        }
